...
WEBHOOK_URL = "" # Entre ici le lien de ton webhook discord
...
def format_discord_embed(symbol, funding_rate, next_funding_rate_time, plus=""):
    timestamp = f"<t:{int(next_funding_rate_time / 1000)}:R>" if next_funding_rate_time else "Non disponible"
...
def post_discord(embed):
    ...
    print(f"{current_time} • {'Succès' if response.status_code == 204 else f'Erreur : {response.status_code} - {response.text}'}")
def execute_script():
    response = fetch_data()
    if response.status_code == 200:
        ...
        current_time = datetime.now().strftime("%H:%M:%S")
        embed = {
            "description": "<:mexc:1279203892197855312> **Taux de financement MEXC**",
            "color": 0x2c75ff,
            "fields": [{"name": "<:positif:1279203868172878048>", "value": positive_rates},
                       {"name": "<:negatif:1279203880583827507>", "value": negative_rates}],
            "footer": {"text": f"@Cryptotenyx X Zayga • Aujourd’hui à {current_time}"}
        }
        post_discord(embed)
    else:
